...
def setup_configuration():
    ...
    # PERBAIKAN DI SINI (Memastikan False ada di baris yang sama)
...
def main():
    print("=== ROBLOX MANAGER (ADB MODE - NO AUTO CONNECT) ===")
    
    # Cek ADB Status
    print("🔍 Memeriksa status ADB...")
    check_cmd = f"adb devices | grep {ADB_ADDRESS}"
    output = os.popen(check_cmd).read()
    
    if ADB_ADDRESS not in output or "offline" in output:
        print("\n❌ ADB BELUM TERHUBUNG!")
        print(f"Script mendeteksi {ADB_ADDRESS} belum siap.")
        print("Solusi:")
        print("1. Jalankan dulu: ./konek.sh")
        print("2. Baru jalankan script ini lagi.")
        return
    else:
        print("✅ ADB Terdeteksi Siap.")
